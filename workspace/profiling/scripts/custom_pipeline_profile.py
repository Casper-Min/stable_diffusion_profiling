...
def main():
    ...
    # 2. Construct custom stable diffusion pipeline
    ...
    # 2-1. Load the autoencoder model which will be used to decode the latents into image space. 
    ...
    # 2-2. Load the tokenizer and text encoder to tokenize and encode the text.
    tokenizer = CLIPTokenizer.from_pretrained(model_id, subfolder="tokenizer")
    text_encoder = CLIPTextModel.from_pretrained(model_id, subfolder="text_encoder")
    # 2-3. The UNet model for generating the latents.
    unet = UNet2DConditionModel.from_pretrained(model_id, subfolder="unet")
    # 2-4. The scheduling algorithm used to progressively add noise to the image during training.
    scheduler = PNDMScheduler.from_pretrained(model_id, subfolder="scheduler")

    # Or use pre-defined stable diffusion pipeline 
    # pipeline: text_encoder&tokenizer + unet&scheduler + vae
    ...
    torch_nvtx.mark("start")
    pr.enable()  # start annotating function calls

    cuda_nvtx.push_range(message="VAE_model_load", color="blue")
    vae.to(torch_device)
    cuda_nvtx.pop_range()
    cuda_nvtx.push_range(message="CLIO_model_load", color="red")
    text_encoder.to(torch_device)
    cuda_nvtx.pop_range()
    cuda_nvtx.push_range(message="UNET_model_load", color="green")
    unet.to(torch_device)
    cuda_nvtx.pop_range() 

    # OR load pre-defined stable diffusion pipeline
    ...
    # 5-2. initialize the scheduler with our chosen num_inference_steps
    cuda_nvtx.push_range(message="init_scheduler", color="green")
    scheduler.set_timesteps(num_inference_steps)
    cuda_nvtx.pop_range()
    
    # 5-3. denoising loop
    # denoise_loop = cuda_nvtx.start_range(message=f"denoise_loop", color="blue")
    cuda_nvtx.push_range(message=f"denoise_loop", color="red")
    ii = 0
    for t in tqdm(scheduler.timesteps):
        
        cuda_nvtx.push_range(message=f"loop_{ii}", color="blue")
        
        # 6-3-1. expand the latents if we are doing classifier-free guidance to avoid doing two forward passes.
        cuda_nvtx.push_range(message=f"preprocess_latent_{t}", color="red")
        latent_model_input = torch.cat([latents] * 2)
        latent_model_input = scheduler.scale_model_input(latent_model_input, timestep=t)
        cuda_nvtx.pop_range()
        
        # 5-3-2. UNET : predict the noise residual
        # with torch.no_grad():
        ...
        # 5-3-3. perform guidance
        cuda_nvtx.push_range(message=f"guidance_{ii}", color="blue")
        noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
        cuda_nvtx.pop_range()
        
        # 5-3-4. compute the previous noisy sample x_t -> x_t-1
        cuda_nvtx.push_range(message=f"compute_denoise_{ii}", color="red")
    ...
    # 6. Decode the image 
    cuda_nvtx.push_range(message="decode_latent", color="blue")
    ...
    cuda_nvtx.push_range(message="post-process", color="red")
    ...
    grid.save(f"custom_result_step_{num_inference_steps}.png")
    # pil_images[0].save(f"result_step_2{num_inference_steps}.png")
